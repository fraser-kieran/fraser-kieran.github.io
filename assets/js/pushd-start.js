...
function savePrefs(){
    ...
    if(gender==null || ages==null || settings==null || emoji==null || sub==null || use==null || entic==null){
        $.alert({
            title: 'Wait!',
            type: 'orange',
    ...
    savePreferences(prolificId, gender, ages, settings, emoji, sub, use, entic, topics, posWeb, negWeb, opened, dismissed)
...
/* Use cookies to get the userId */
function savePreferences(prolificId, gender, ages, settings, emoji, sub, use, entic, topics, posWeb, negWeb, opened, dismissed) {
    ...
    if(userId.charAt(0) == '-'){
        database.ref('participant/'+userId).update({
            ...
            prestudyCompleted: Math.round((new Date()).getTime())
        });
    }
}
